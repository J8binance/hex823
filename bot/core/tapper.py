...
class Tapper:
    ...
    async def game_sign(self, http_client: aiohttp.ClientSession, proxy: Proxy) -> None:
        try:
            proxies = {
                "http": proxy,
                "https": proxy
            ...
            if check_proxy_availability.status_code == 200:
                ...
                logger.info(f"sign {self.session_name} | Proxy IP: {ip}")
            result = requests.get("https://ago-api.hexacore.io/api/daily-checkin", proxies=proxies, headers=head)
            sign_json = result.json()
            if sign_json.get('available_at'):
                is_available = sign_json.get('is_available')
                next = sign_json.get('next')
                if is_available:
                    param = {"day": next}
                    logger.info(
                        f"<light-yellow>{self.session_name}</light-yellow> |  sign： {next} day")
                    res = requests.post(url=f'https://ago-api.hexacore.io/api/daily-checkin', json=param,
                                        proxies=proxies, headers=head)
                    res_json = res.json()
                    if res_json.get('success'):
                        logger.info(
                            f"<light-yellow>{self.session_name}</light-yellow> |   sign success")
                        return True
                    else:
                        return False
        except Exception as error:
            logger.error(f"<light-yellow>{self.session_name}</light-yellow> | Error while sign {error}")
            ...
            try:
                ...
                if status is not False:
                    logger.success(f'<light-yellow>{self.session_name}</light-yellow> | Daily checkin claimed, '
                                   f'streak - {day}')
                if settings.AUTO_SIGN:
                    await self.game_sign(http_client=http_client, proxy=proxy)
